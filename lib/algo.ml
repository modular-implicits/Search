...
let rec dfsMF  : {O : Ord} -> {M : Monad} -> {F : Foldable} -> (O.t -> (O.t F.t) M.t) -> (O.t -> bool M.t) -> (O.t) -> ((O.t list) option) M.t = 
        fun {O : Ord} {M : Monad} {F : Foldable} (f : O.t -> (O.t F.t) M.t) (pred : O.t -> bool M.t) (start : O.t) -> let module X = DFSF (O) (M) (St.Make{O}) (F) in  X.dfsM f pred start (St.empty {St.Make{O}})
